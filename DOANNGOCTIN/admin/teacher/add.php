...
$id = $name =$service=$image=$price=$date=$gender=$address=$phone='';
if (!empty($_POST)) {
	if (isset($_POST['name'])) {
		$name = $_POST['name'];
	...
	if (!empty($name)) {
		...
		//Luu vao database
		if ($id == '') {
			$sql = 'insert into teacher(name, service, image, class_id, date, gender, address, updated_at, phone) values ("'.$name.'", "'.$service.'", "'.$image.'", "'.$date.'", "'.$updated_at.'", "'.$gender.'", "'.$address.'", "'.$phone.'")';
		} else {
			$sql = 'update teacher set name = "'.$name.'", service="'.$service.'", updated_at = "'.$updated_at.'", image = "'.$image.'",  phone = "'.$phone.'", address = "'.$address.'", gender = "'.$gender.'", date = "'.$date.'"  where id = '.$id;
		}
...
">
		<div class="panel panel-primary" style="background:#0099ff;color: white; padding: 15px;">
			<div class="panel-heading">
				<h2 class="text-center">THÊM/SỬA GIÁO VIÊN</h2>
			</div>
