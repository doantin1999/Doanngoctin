...
if (isset($_GET['id'])) {
    ...
    $loginstudent = executeSingleResult($sql);
...
<ul class="nav nav-tabs">
     <li class="nav-item">
      ...
      <li class="nav-item">
       ...
       <li class="nav-item">
        <a class="nav-link active" href="#"style="background:#0099ff;color: white;font-size: 20px">TÀI KHOẢN</a>
      </li>
       <li class="nav-item">
...
$sql         = 'select * from loginstudent where 1 '.' limit '.$firstIndex.', '.$limit;
$productList = executeResult($sql);

$sql         = 'select count(id) as total from loginstudent where 1 ';
  ...
  if (isset($_REQUEST['ok'])) 
        {
            ...
            {
                $sql = "select * from loginstudent where name like N'%$search%' OR email like N'%$search%'";
...
<script type="text/javascript">
		function deleteProduct(id) {
			var option = confirm('Bạn có chắc chắn muốn xóa người này không?')
			if(!option) {
				return;
			}
