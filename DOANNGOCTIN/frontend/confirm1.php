...
<br>
  <div style="background-color: #0099ff; width: 100%; height: 100%; margin-right: 0px;padding: 20px;color: white;" class="panel-body">
            ...
            <h2 class="text-center">Xác nhận nhập học</h2>
			</div>
			<div class="panel-body">
				<div class="row">
           <form method="get">
              <div class="form-group" style="width: 400px; float: right;">
                <input type="text" class="form-control" placeholder="Nhập tên hoặc email của bạn..." name="search">
...
$sql         = 'select * from loginstudent1 where 1 '.' limit '.$firstIndex.', '.$limit;
$productList = executeResult($sql);

$sql         = 'select count(id) as total from loginstudent1 where 1 ';
  ...
  if (isset($_REQUEST['ok'])) 
        {
            ...
            {
                $sql = "select * from loginstudent1 where email like N'%$search%' or name like N'%$search%'";
    ...
    echo '<tr style="color: white;">
       <td>'.(++$firstIndex).'</td>
        ...
        <td>'.$item['address'].'</td>
        <td>'.$item['phone'].'</td>
        <td>'.$item['subjects']." ".$item['subjects1']." ".$item['subjects2'].'</td>
        <td>'.$item['class'].'</td>
        <td>'.$item['school'].'</td>
      </tr>';
                    }
                    ...
                    echo "Không có bạn trong danh sách!";
                }
            }
        }
?>
