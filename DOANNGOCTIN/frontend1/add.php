...
if (!empty($_POST)) {
  ...
  if (!empty($name)) {
    ...
    //Luu vao database
        $sql = 'update loginstudent set name = "'.$name.'", email="'.$email.'", date1="'.$date.'", gender = "'.$gender.'", image = "'.$image.'",  address = "'.$address.'", phone = "'.$phone.'", updated_at = "'.$updated_at.'" where id = '.$id;
...
if (isset($_GET['id'])) {
  $id      = $_GET['id'];
  $sql     = 'select * from loginstudent where id = '.$id;
  $loginstudent1 = executeSingleResult($sql);
  if ($loginstudent1 != null) {
    $name       = $loginstudent1['name'];
    $email       = $loginstudent1['email'];
    $date       = $loginstudent1['date1'];
    $gender       = $loginstudent1['gender'];
    $image   = $loginstudent1['image'];
    $address = $loginstudent1['address'];
    $phone = $loginstudent1['phone'];
  }
}
?>
<!DOCTYPE html>
  ...
  <div style="background-color: #0099ff; width: 100%; height: 100%; margin-right: 0px;padding: 20px;color: white;" class="panel-body">
      ...
      <div class="panel-body">
          ...
          <label for="phone">Số điện thoại:</label>
           <input required="true" type="text" class="form-control" id="phone" name="phone" value="<?=$phone?>"></br>
          </div>
          <div class="form-group">
